...
test('test', async ({ page }) => {
  ...
  await expect(page.locator('li').filter({ hasText: 'test 2 Abstract from the' })).toBeVisible();

  await page.locator('#content div').filter({ hasText: 'test 2 Abstract from the' }).nth(2).click();
