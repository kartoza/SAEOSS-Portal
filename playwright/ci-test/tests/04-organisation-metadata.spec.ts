...
test('add organisation metadata', async ({ page }) => {
  ...
  await page.getByPlaceholder('eg. A descriptive title').fill('test metadata');
  ...
  await page.getByPlaceholder('eg. Some useful notes about').fill('test metadata');
  ...
  await page.getByLabel('* Spatial resolution').fill('1:2000');
  ...
  await page.getByLabel('* Reference datetime').fill('2024-01-25T11:05');
  
  await page.getByLabel('* Metadata stamp date').click();
  await page.getByLabel('* Metadata stamp date').fill('2024-01-25T11:12');
  ...
  await fileChooser.setFiles('tests/fixtures/data.json');
  ...
  await expect(page.getByRole('heading', { name: 'test metadata' })).toBeVisible();
